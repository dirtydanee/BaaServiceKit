...
class ConfigTests: XCTestCase {
    ...
    func testConfigModel() throws {
        ...
        let configModel = Config(chainpointConfigResponse: configResponse)
        XCTAssertEqual(configModel.version, configResponse.version)
        XCTAssertEqual(configModel.proofExpireMinutes, configResponse.proofExpireMinutes)
        XCTAssertEqual(configModel.getProofsMaxRest, configResponse.getProofsMaxRest)
        XCTAssertEqual(configModel.postHashesMax, configResponse.postHashesMax)
        XCTAssertEqual(configModel.postVerifyProofsMax, configResponse.postVerifyProofsMax)
        XCTAssertEqual(configModel.time, configResponse.time)
